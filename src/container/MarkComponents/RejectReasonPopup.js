...
function RejectReasonPopup(props) {
  ...
  const { showV5SelectFilter, showThreeLevelAbbreviation, labelTreeList, asyncGetTop20List } = rejectPopupConfig || {};
  ...
  const filteredReasons = filterReason(getFinalReasons(), true, reasonFilters, keyWordsStr, labelList);
  ...
  let count;
  if (needLevelReason) {
    ({ reasons: afterInterceptReasons, count } = levelReason(filteredReasons, allReasons, keyWordsStr));
  } else {
  ...
  useEffect(() => {
    if (!isEqual(prevElementRef.current, element)) {
      ...
      if (channelElementReviewInfoMap) {
        ...
        if (channelReviewInfo && channelReviewInfo.rejectReasonLabels && channelReviewInfo.rejectReasonLabels.length) {
      ...
      const elementTypeFilter = newReasonFilters.find((item) => item.field === 'applyElementTypeList');
    ...
    <div className={styles.container} onClick={onClickDeal} onDoubleClick={onClickDeal}>
      ...
      <div className={styles['smart-labels-bar']}>
            ...
            {filterLabels.map((item, index) => (
              <Link
                ...
                className={classNames({
                  [styles.active]: reasonFilters.find((f) => item.field === f.field && item.value === f.value),
                })}
                onClick={() => {
                  const newReasonFilters = [...reasonFilters];
                  const thisFieldFilter = newReasonFilters.find((innerItem) => innerItem.field === item.field);
                  if (thisFieldFilter) {
                    if (thisFieldFilter.value === item.value) {
                      newReasonFilters.splice(newReasonFilters.indexOf(thisFieldFilter), 1);
                    } else {
                      newReasonFilters[newReasonFilters.indexOf(thisFieldFilter)].value = item.value;
                    }
                  } else {
                    newReasonFilters.push({
                      type: 'field',
        ...
        {isFunction(asyncGetTop20List) && (
          <div>
            <Tag shape="round" theme="primary" variant="light" style={{ cursor: 'pointer' }} onClick={getTop20List}>
              ...
              <div key={reviewPolicyId || index} className={styles['reason-section']}>
                {threeLevelReasonName && (
                  <p className={styles['level-3']}>
                    <HighlightFragment description={threeLevelReasonName} keyWordStr={keyWordsStr} />
                  </p>
                )}
                <ReasonList
                  reasons={children}
